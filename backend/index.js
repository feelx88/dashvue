...
app.post('/api/login', (req, res) => {
    for (const login of config.logins) {
        if (login.username === req.body.username && login.password === req.body.password) {
            let token = jwt.sign({ authenticated: true, nonce: uuidv4() }, config.secret, { expiresIn: '100 years' });
...
app.get('/api/config',  passport.authenticate('jwt', { session: false }), (req, res) => {
